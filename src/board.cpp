...
Board::Board(int offsetX, int offsetY) : mOffset{Vector2{(float)offsetX, (float)offsetY}}
{
}
            ...
            for (int y = 0; y < 10; y++)
            {
                switch (mCells[x][y])
                {
                    ...
                    case CellState::SHIP_1_HITTED:
                        DrawTexture(textures[CellState::SHIP_1_HITTED], mOffset.x + (45 * x), mOffset.y + (45 * y) + 1,
                                    WHITE);
                        break;

                    case CellState::SHIP_2_HITTED:
                        DrawTexture(textures[CellState::SHIP_2_HITTED], mOffset.x + (45 * x), mOffset.y + (45 * y) + 1,
                                    WHITE);
                        break;

                    case CellState::SHIP_3_HITTED:
                        DrawTexture(textures[CellState::SHIP_3_HITTED], mOffset.x + (45 * x), mOffset.y + (45 * y) + 1,
                                    WHITE);
                        break;

                    case CellState::SHIP_4_HITTED:
                        DrawTexture(textures[CellState::SHIP_4_HITTED], mOffset.x + (45 * x), mOffset.y + (45 * y) + 1,
                                    WHITE);
                        break;
